...
/* Should the YEETSH internal functions dump verbose output? */
static int yeetsh_internal_verbose = 0;

void maybe_print_error() {
  if (yeetsh_internal_verbose) {
    char *err = strerror(errno);
    printf("[YEETSH INTERNAL ERROR]: %s\n", err);
  }
}
...
void Closedir(DIR *dirp) {
  if (closedir(dirp) == -1 && yeetsh_internal_verbose) {
    printf("[YEETSH_INTERNAL]: Error closing directory.\n");
  }
}
